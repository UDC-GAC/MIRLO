...
def generate_folds(pairs):
    ...
    for pair in pairs:
        if not pair[0] in protein_interactions:
            protein_interactions[pair[0]] = 0
        protein_interactions[pair[0]] += 1

    protein_interactions = sorted(protein_interactions.items(), key=lambda x: x[1], reverse=True)

    folds = [[] for _ in range(5)]
    for k, (protein, _) in enumerate(protein_interactions):
        for pair in pairs:
            if pair[0] == protein:
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        for k, fold in enumerate(folds):
            fold_path = os.path.join(dataset_path, 'kfold_no_share_protein', f'{k}')
